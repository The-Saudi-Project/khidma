        ...
        <div className="space-y-6 sticky top-24">
          ...
          <div className="glass rounded-3xl p-5 border border-white/5 shadow-2xl space-y-2 text-xs">
            ...
            {booking.customer?.phone && (
              <p className="text-[#22C55E] font-mono text-[11px] font-bold pt-1">{booking.customer.phone}</p>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass rounded-3xl p-6 lg:p-8 max-w-md w-full border border-white/5 shadow-2xl animate-scale-in space-y-4">
            ...
            <div className="flex gap-2.5 pt-2">
              <button onClick={() => setShowRejectModal(false)} type="button" className="flex-1 py-2.5 rounded-2xl bg-white/5 hover:bg-slate-200 text-slate-300 font-bold text-xs transition-colors">
                Cancel
              </button>
              <button onClick={handleRejectPayment} disabled={rejectingPayment || !rejectReason.trim()} type="button"
                className="flex-1 py-2.5 rounded-2xl bg-red-600 hover:bg-red-700 text-white font-extrabold text-xs transition-colors shadow-2xl flex items-center justify-center gap-1.5 disabled:opacity-50">
